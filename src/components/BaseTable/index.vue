<template>
  <div class="base-table">
    <ul class="base-table-header">
      <li v-for="(item, index) in columns" :key="index" class="base-table-header-column" :style="{ width: item.width }">
        {{ item.title }}
      </li>
    </ul>
    <div v-autoscroll="20" class="base-table-body">
      <ul v-for="(row, index1) in dataSource" :key="index1" class="base-table-body-row">
        <li v-for="(column, index2) in row" :key="index2" class="base-table-body-column"
